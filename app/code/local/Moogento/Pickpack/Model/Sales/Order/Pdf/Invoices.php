...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices extends Moogento_Pickpack_Model_Sales_Order_Pdf_Aabstract
{
    ...
    protected $_page_top = 0;
    protected $_padded_right = 0;
    protected $_padded_left = 0;
	...
	protected function getMaxCharMessage2($message, $font_size_options, $font_temp, $message_box_left){
		$maxWidthPage_message = $this->_padded_right - $message_box_left - 10;
	...
	protected function getMaxCharMessage($message_box_right, $font_size_options, $font_temp, $message_box_left=30){
		$maxWidthPage_message = $message_box_right - $message_box_left;
		$font_temp_message = Zend_Pdf_Font::fontWithName(Zend_Pdf_Font::FONT_HELVETICA);
		$font_size_compare_message = $font_size_options;
    ...
    protected function printBackGroundImage($page, $store_id, $page_background_image_yn, $full_page_width, $page_background_position, $sub_folder, $option_group, $suffix_group, $x1, $y2, $page_background_nudge,$page_background_resize){
            ...
            if ($filename) {
                ...
                if (is_file($image_path)) {
                    ...
                    if ($orig_img_width > $full_page_width) {
                        ...
                        if ($orig_img_height > $this->_page_top) {
                            $temp_var = $this->_page_top / $orig_img_height;
                            $img_height = $this->_page_top;
                    ...
                    if($page_background_position == 'topleft'){
                        ...
                        $y2 = ($this->_page_top + 10 - $img_height) / 2 + $img_height;
                    }else{
                        $x1 = ($full_page_width - $img_width) / 2;
                        if($page_background_resize == 'high')
                            $y2 = ($this->_page_top - 200);
                        else
                            $y2 = ($this->_page_top - 350);
                    }
	...
	protected function printHeaderLogo($page,$store_id, $show_top_logo_yn, $_logo_maxdimensions, $sub_folder, $option_group, $suffix_group, $x1 = 27, &$y2){
		// require_once Mage::getBaseDir('app') . '/code/local/Moogento/Pickpack/Model/Sales/Order/Pdf/lib/Zebra_Image.php';
        require_once Mage::getBaseDir('app') . '/code/local/Moogento/Pickpack/Model/Sales/Order/Pdf/lib/Simple_Image.php';
		// $image_zebra = new Zebra_Image();
        $image_simple = new SimpleImage();
		...
		$minY_logo = $this->_page_top; //$this->_printing_format['page_top'];
...
//    }
    ...
    public function printBottomOrderId($order,$page,$font_style_body,$font_size_body,$font_family_body,$font_color_body,$font_size_shipaddress,$store_id){
        if (($this->_bottom_shipping_address_yn == 1)&&($this->_bottom_shipping_address_id_yn == 1)) {
            if($this->_packingsheet['case_rotate_address_label'] > 0)
                $this->rotateLabel($this->_packingsheet['case_rotate_address_label'],$page, $this->_nudge_rotate_address_label);
            ...
            $page->drawText('#' . $order->getRealOrderId(), $this->_packingsheet['pickpack_shipaddress'][0] + $bottom_order_id_nudge[0], $bottom_order_id_nudge[1] + $this->_packingsheet['pickpack_shipaddress'][1], 'UTF-8');
            $minY[] = ($this->_packingsheet['pickpack_shipaddress'][1] + ($font_size_shipaddress)) - 7;
            unset($bottom_order_id_nudge);
            if($this->_packingsheet['case_rotate_address_label'] > 0)
                $this->reRotateLabel($this->_packingsheet['case_rotate_address_label'],$page,$this->_nudge_rotate_address_label);
        }
    }
    ...
    public function setPickPackInvoiceConfig($store_id){
        ...
        /*************************** DEFAULT SETTING FOR EACH PAGE SIZE ***************************/
        ...
        /*************************** END DEFAULT SETTING FOR EACH PAGE SIZE ***************************/

        /*************************** TOP PAGE ***************************/
        $this->_packingsheet['capitalize_label_yn'] = $this->_getConfig('capitalize_label_yn', 0, false, $this->_wonder, $store_id); // o,usonly,1
        $this->_packingsheet['capitalize_label2_yn'] = $this->_getConfig('capitalize_label2_yn', 0, false, $this->_wonder, $store_id); // o,usonly,1
        /*************************** END TOP PAGE ***************************/

        /*************************** BOTTOM PAGE ***************************/
        $this->_packingsheet['case_rotate_address_label'] = $this->_getConfig('case_rotate_address_label',0, false, $this->_wonder, $store_id);
        $this->_packingsheet['pickpack_return_address_yn'] = $this->_getConfig('pickpack_return_address_yn', 0, false, $this->_wonder, $store_id);
        $this->_packingsheet['nudge_entire_bottom'] = explode(',',$this->_getConfig('nudge_entire_bottom', '0,0', false, $this->_wonder, $store_id));
        $this->_packingsheet['pickpack_show_full_payment_yn'] = $this->_getConfig('pickpack_show_full_payment_yn', 0, false, $this->_wonder, $store_id);
        $this->_packingsheet['show_shipping_method_bottom_yn'] = $this->_getConfig('show_shipping_method_bottom_yn', 0, false, $this->_wonder, $store_id);
        $this->_packingsheet['show_shipping_method_bottom_nudge'] = explode(",", $this->_getConfig('show_shipping_method_bottom_nudge', '0,0', true, $this->_wonder, $store_id));
        $this->_packingsheet['pickpack_bottom_shipping_address_yn'] = $this->_getConfig('pickpack_bottom_shipping_address_yn', 0, false, $this->_wonder, $store_id);
        //missing capitalize_label2_yn
        $this->_packingsheet['pickpack_shipaddress'] = explode(",", $this->_getConfig('pickpack_shipaddress', $addressFooterXYDefault, true, $this->_wonder, $store_id));//$addressFooterXY $this->_addressFooterXY
        $this->_packingsheet['pickpack_shipaddress'][0] += $this->_packingsheet['nudge_entire_bottom'][0];
        $this->_packingsheet['pickpack_shipaddress'][1] += $this->_packingsheet['nudge_entire_bottom'][1];
        /*************************** END BOTTOM PAGE ***************************/
    }

    private function reRotateLabel($case_rotate,&$page,$nudge_rotate_address_label)
    {
        $x = -155;
        $y = -55;
        ...
        $page->rotate($this->_page_top/2+$nudge_rotate_address_label[0],$this->_padded_right/2 +$nudge_rotate_address_label[1], 0-$rotate);
    }
     ...
     *
     */
    protected function printBundleShelving($page,$shelving_yn,$shelving_attribute,$product,$child,$columns_xpos_array,$shelfX,$addon_shift_x,$store_id,$page_count,$items_header_top_firstpage,$order_number_display)
    {
        $helper = Mage::helper('pickpack');
        ...
        if ( $shelving_real_yn == 1 && $shelving_yn == 1 && $product->offsetExists($shelving_attribute)) {
            ...
            $next_col_to_shelving_real = getPrevNext2($columns_xpos_array,$shelfX, 'next', $this->_padded_right);
    ...
    public function isInShippingZone($order){
        if((isInstalled('Moogento_CourierRules')))
        {
            $store_id               = $order->getStore()->getId();
     ...
     */
    public function splitWorldsToArrayBaseOnMaxWidth($message_string, $max_width = 250, $font_size = 10, $font_temp = null)
    {
