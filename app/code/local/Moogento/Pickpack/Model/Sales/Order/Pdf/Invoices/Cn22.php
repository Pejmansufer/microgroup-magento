		...
		$need_new_page = 1;
         foreach ($orderCollection as $order) {
			 if(($number_printed >= $numger_of_label_on_sheet) || ($need_new_page == 1))
			 {
				...
				// if($first_page == 1)
// 					if($case_rotate > 0)
// 						$this->reRotateLabel($case_rotate,$page,$this->page_top,$this->padded_right,$nudge_rotate_address_label);
				if($case_rotate > 0)
					$this->rotateLabel($case_rotate,$page,$this->page_top,$this->padded_right,$nudge_rotate_address_label);	
// 				$first_page = 1;
			 } 
			
            // echo $current_label_xpos.' -- '.$current_label_ypos.'<br/>';
            // echo $label_height.' -- '.$label_width; exit;
			try{
				$custom_section_model->printCustomSection(0,$page,$order,'cn22',$current_label_xpos,$current_label_ypos,$item_qty_array,$label_width,$label_height);
     ...
     * */
    public function printOneLabelCN22($page,$order,$store_id, $nudgeX = 0, $nudgeY = 0,$rotate = 0){
        //get config
        $label_width = $this->_getConfig('label_width', '279', false,'cn22_label', $store_id,true,'cn22_options');
        $label_height = $this->_getConfig('label_height', '245', false,'cn22_label', $store_id,true,'cn22_options');
        ...
        /** get position to print CN22 */
        $_Xpos = $nudgeX;
        $_Ypos = $page->getHeight() - $nudgeY;
        if ($rotate == 1){
            $rotate = deg2rad(90);
            $_Xpos = $nudgeY;
            $_Ypos = $nudgeX;
            ...
            $rotate = deg2rad(-90);
            $_Xpos = $page->getWidth() - $nudgeY;
            $_Ypos = $page->getHeight() - $nudgeX;
        }

        try{
            $custom_section_model = new Moogento_Cn22_Model_Pdf();
            $custom_section_model->printCustomSection($rotate,$page,$order,'cn22',$_Xpos,$_Ypos,$item_qty_array,$label_width,$label_height);
        }
        catch(Exception $e)
        {
            echo $e->getMessage(); exit;
