        ...
        if ($batchId = $this->getRequest()->getParam('id')) {
            ...
            if ($batchModel->getId()) {
                ...
                try {
                    ...
                    $result['error'] = Mage::helper('adminhtml')->__('An error occurred while finishing processing. Please refresh the cache.');
                }
                $batchModel->delete();
                $this->getResponse()->setBody(Mage::helper('core')->jsonEncode($result));
            }
    ...
    private function _savedImportShipment()
    {
        ...
        if($session->getData('import_data_for_save')){
            ...
            foreach($orders_data as $order_id => $order_data){
                $order = Mage::getModel('moogento_shipeasy/sales_order')->load($order_id, 'increment_id');
