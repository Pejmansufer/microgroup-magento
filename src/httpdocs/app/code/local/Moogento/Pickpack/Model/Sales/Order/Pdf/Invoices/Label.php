...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Label extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices
{
    ...
    public function getLabel($orders = array())
    {
        ...
        */
        ...
        $nudgel_logo_background = explode(",", $this->_getConfig('label_nudgelogo_text', '0,0', false, 'label', $store_id)); //0,0
        ...
        foreach ($orders as $orderSingle) {
            ...
            if (!$useGFSLabel) {
                ...
                $address_format_set = ltrim($address_format_set, ',');
                $address_format_set = rtrim($address_format_set, ',');
                $shippingAddressArray = explode("\n", $address_format_set);
                foreach ($shippingAddressArray as $k => $v) {


                    if (strlen(trim($v)) == 0)
                        unset($shippingAddressArray[$k]);
                    else {
                        $value_shipping = rtrim($v, ',');
                        $value_shipping = ltrim($value_shipping, ",");
                        $value_shipping = ltrim($value_shipping, ".");
                        $value_shipping = trim($value_shipping);
                        $value_shipping = trim(str_replace(',,', ',', $value_shipping));
                        $value_shipping = trim(str_replace(', ,', ',', $value_shipping));
                        $value_shipping = Mage::helper('pickpack/functions')->clean_method($value_shipping, 'pdf');
                        $shippingAddressArray[$k] = ucwords($value_shipping);
                    }
                }
...
//sac
                /***************** PRINT SHIPPING ADDRESS BACKGROUND********************/
                if (isset($label_logo_yn) && $label_logo_yn == 1) {
                    $this->printShippingPostageLabel($order, $page, $temp_x, $temp_y);
                }
                /***************** END PRINT SHIPPING ADDRESS BACKGROUND********************/
                ...
                foreach ($shippingAddressArray as $i => $value) {
                    ...
                    while ($token != false) {
                        ...
                        /* echo '<pre>';
                         var_dump($address_line_y);
                         var_dump($line_height_top);
                         var_dump($i_space);
                         var_dump($line_addon);
                         var_dump($token_addon);
                         var_dump($this_address_line_y);
                         var_dump($paper_margin);
                         echo '</pre>';*/
                ...
                if (($customer_phone_yn != 'no') && ($customer_phone != '')) {
                    ...
                    $address_line_x = $temp_x + $line_addon;
                    ...
                    if ($customer_phone_yn != 'yesdetails') {
                        ...
                        $page->drawText($value, $address_line_x + 97, $address_line_y, 'UTF-8');
    ...
    protected function printShippingAddressBackgroundImage($order, $scale = 100, $shipping_address_background, $page, $x1 = 0, $y1 = 0, $x2 = null, $y2 = null)
    {
            ...
            if ($image_file_name) {
                ...
                if ((($image_ext == 'jpg') || ($image_ext == 'jpeg') || ($image_ext == 'png')) && (is_file($image_file_name))) {
                    ...
                    if (isset($image_simple)) {
                        ...
                        if (!(file_exists($image_target))) { //check if cache image was exit or not
                            $image_simple->load($image_source);
                            $image_simple->resize($img_width1, $img_height1);
                            $image_simple->save($image_target, IMAGETYPE_JPEG, 100); //create new cache for use later
                        }
                        $image_file_name = $image_target;
                    }
                    $image = Zend_Pdf_Image::imageWithPath($image_file_name);

                    //calculate potition to print image
                    $x1 = $x1 + $shipping_background_nudge_x;
                    $y1 = $y1 + $shipping_background_nudge_y;
                    if (isset($x2)){
                        $x2 = $x2 + $shipping_background_nudge_x;
                    }else $x2 = $x1 + $img_width + $shipping_background_nudge_x;
                    if (isset($y2)){
                        $y2 = $y2 + $shipping_background_nudge_y;
                    }else $y2 = $y1 + $img_height + $shipping_background_nudge_y;
                    //end calculate potition to print image
...
//sac
    private function printShippingPostageLabel($order, $page, $temp_x, $temp_y)
    {
        $store_id = $order->getStore()->getId();
        ...
        $this->printShippingAddressBackgroundImage($order, $scale = 100, $shipping_address_background, $page, $x1, $y1,$x2 ,$y2);
