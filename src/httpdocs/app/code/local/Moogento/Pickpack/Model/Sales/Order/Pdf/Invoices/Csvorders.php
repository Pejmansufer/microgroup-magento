    ...
    public function getCsvOrders($orders = array(), $from_shipment = 'order')
    {
	    ...
	    foreach ($order_collection as $order) {                   
            ...
            if ($order->getCreatedAtStoreDate())
                $row[$order_id]['order_date'] 		= Mage::getSingleton('core/date')->date($date_format,$order->getCreatedAtStoreDate());
            ...
            if( isset($row[$order_id]['coupon_code']) && $row[$order_id]['coupon_code'] != '' ){
                  $coupon_rule = Mage::getModel('salesrule/coupon')->loadByCode($row[$order_id]['coupon_code']);
