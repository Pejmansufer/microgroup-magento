...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Separated extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices
{
    ...
    {
        $shelving = $this->getProductAttributeValue($product, $shelving_attribute, false);
        if (trim($shelving) != '') {
    ...
    private function getCustomAttributeBundle($product_child, $shelving_attribute, $shelving_real_b)
    {
        if ($product_child->getData($shelving_attribute)) {
            $shelving_real_b = $this->getProductAttributeValue($product_child, $shelving_attribute, false);
        } else {
        ...
        if (is_array($shelving_real_b)) $shelving_real_b = implode(',', $shelving_real_b);
    ...
    function getPickSeparated($orders = array(), $from_shipment = 'order')
    {
        ...
        $background_color_subtitles_default = '#CCCCCC';
        ...
        $this->_general['font_family_subtitles'] = $this->_getConfig('font_family_subtitles', $font_family_subtitles_default, false, 'general');
        $this->_general['font_style_subtitles'] = $this->_getConfig('font_style_subtitles', $font_style_subtitles_default, false, 'general');
        $this->_general['font_size_subtitles'] = $this->_getConfig('font_size_subtitles', $font_size_subtitles_default, false, 'general');
        $this->_general['font_color_subtitles'] = $font_color_subtitles_default;//trim($this->_getConfig('font_color_subtitles', $font_color_subtitles_default, false, 'general'));
        $background_color_subtitles = $background_color_subtitles_default;//trim($this->_getConfig('background_color_subtitles', $background_color_subtitles_default, false, 'general'));
        $this->_general['font_family_body'] = $this->_getConfig('font_family_body', $font_family_body_default, false, 'general');
        $this->_general['font_style_body'] = $this->_getConfig('font_style_body', $font_style_body_default, false, 'general');
        $this->_general['font_size_body'] = $this->_getConfig('font_size_body', $font_size_body_default, false, 'general');
        $this->_general['font_color_body'] = trim($this->_getConfig('font_color_body', $font_color_body_default, false, 'general'));
        ...
        $font_color_body_zend = new Zend_Pdf_Color_Html($this->_general['font_color_body']);

        //$this->_general['non_standard_characters'] = $this->_getConfig('non_standard_characters', 0, false, 'general');
...
{if country}{country}|{/if}";
        ...
        if ($product_images_yn == 1) {
            ...
            if ($product_images_maxdimensions[0] == '' || $product_images_maxdimensions[1] == '') {
                ...
                if ($product_images_maxdimensions[0] == NULL && $product_images_maxdimensions[1] == NULL) $product_images_maxdimensions[0] = 50;
            }
        ...
        elseif ($pickpack_options_filter == '' && $pickpack_options_filter_yn == 1) $pickpack_options_filter_yn = 0;
        ...
        foreach ($orders as $orderSingle) {
            ...
            $font_temp = Zend_Pdf_Font::fontWithName(Zend_Pdf_Font::FONT_HELVETICA);
            $test_name_length = round($this->parseString($test_name, $font_temp, ($this->_general['font_size_body']))); //*0.77)); // bigger = left
            ...
            foreach ($itemsCollection as $item) {
                ...
                if ($stock = Mage::getModel('cataloginventory/stock_item')->loadByProduct($product_id)->getQty()) $stock = round($stock); //Mage::getModel('cataloginventory/stock_item')->loadByProduct($product_id)->getQty());
                ...
                // $gallery_data = $image_product->getData('media_gallery');
                ...
                if ($shmethod == 1) {
                    ...
                    if ($sku_shipping_method[$order_id] != '') $sku_shipping_method[$order_id] = $sku_shipping_method[$order_id];
                    ...
                    $font_temp = Zend_Pdf_Font::fontWithName(Zend_Pdf_Font::FONT_HELVETICA);
                    $font_size_compare = ($this->_general['font_size_subtitles'] - 2);
            ...
            foreach ($supplier_master as $key => $supplier) {
                if ((isset($supplier_login) && ($supplier_login != '') && (strtoupper($supplier) == strtoupper($supplier_login))) || !isset($supplier_login) || $supplier_login == '') {
                    ...
                    if ($picklogo == 1) {
                        ...
                        //770 / 820 ($page_top-5)
                        $this->_setFont($page, $font_style_header, $font_size_header, $font_family_header, $this->_general['non_standard_characters'], $font_color_header);
                        $page->drawText($helper->__('Order-separated Pick List'), 325, ($page_top - 5), 'UTF-8');
                        $page->drawText(Mage::helper('sales')->__($supplier), 325, ($page_top - 25), 'UTF-8');
                        ...
                        $this->_setFont($page, $font_style_header, ($font_size_header + 2), $font_family_header, $this->_general['non_standard_characters'], $font_color_header);
                    ...
                    foreach ($order_id_master as $order_id => $value) {
                        ...
                        if ($each_order_per_page == 1 && ($fist_order == false))
                            ...
                            if ($this->y < $this->_general['font_size_subtitles'] + 20)
                                $condition_new_page = 1;
                        ...
                        if ($supplier_skip_order == FALSE) {
                            ...
                            $this->_setFont($page, 'bold', $this->_general['font_size_subtitles'], $this->_general['font_family_subtitles'], $this->_general['non_standard_characters'], $this->_general['font_color_subtitles']);
                            ...
                            if ($barcodes == 1) {
                                ...
                                $barcodeWidth = $this->parseString($order_id, Zend_Pdf_Font::fontWithPath($this->action_path . $font_family_barcode), 16);
                                $page->setFillColor($white_color);
                                ...
                                if ($grey_title == TRUE) $page->setFillColor(new Zend_Pdf_Color_GrayScale(0.4));
                                $page->setFont(Zend_Pdf_Font::fontWithPath($this->action_path . $font_family_barcode), 16);
                            ...
                            if ($shmethod == 1) {
                                ...
                                if ($grey_title == TRUE) $page->setFillColor(new Zend_Pdf_Color_GrayScale(0.4));
                                $this->_setFont($page, 'bold', ($this->_general['font_size_subtitles'] - 2), $this->_general['font_family_subtitles'], $this->_general['non_standard_characters'], $this->_general['font_color_subtitles']);
                            ...
                            if ($giftwrapyn == 1) {
                                $this->_setFont($page, 'regular', ($this->_general['font_size_subtitles'] - 2), $this->_general['font_family_subtitles'], $this->_general['non_standard_characters'], $this->_general['font_color_subtitles']);
                            ...
                            if ($order_address_yn == 1) {
                                ...
                                if ($grey_title == TRUE) $page->setFillColor(new Zend_Pdf_Color_GrayScale(0.4));
                                $this->_setFont($page, 'regular', ($this->_general['font_size_subtitles'] - 4), $this->_general['font_family_subtitles'], $this->_general['non_standard_characters'], $this->_general['font_color_subtitles']);
                            ...
                            foreach ($sku_order_id_qty[$order_id] as $sku => $qty) {
                                ...
                                if ($supplier_item_action != 'hide' && trim($supplier_item_action) != '') {
                                    ...
                                    if ($from_shipment == 'shipment') {
                                        $max_qty_length_display = ((($max_qty_length + 5) * ($this->_general['font_size_body'] * 1.1)) - 57);
                                    ...
                                    if ($show_bundle_parent) {
                                        if ($qty_string <= 1 || $product_qty_upsize_yn == '0') {
                                            //set normal font for printing qty <=1 or when upsize qty wasn't set
                                            $this->_setFont($page, $this->_general['font_style_body'], $this->_general['font_size_body'], $this->_general['font_family_body'], $this->_general['non_standard_characters'], $this->_general['font_color_body']);
                                        }else if ($qty_string > 1 && $product_qty_upsize_yn != '0') {
                                            $this->_setFont($page, 'bold', $this->_general['font_size_body'], $this->_general['font_family_body'], $this->_general['non_standard_characters'], $this->_general['font_color_body']);
                                            $qty_draw_width = $this->widthForStringUsingFontSize($qty_string, $page->getFont(), $page->getFontSize());
                                            if ($product_qty_upsize_yn == '1') { //boxed option
                                                $page->setFillColor($black_color);
                                                //draw box
                                                $page->drawRectangle(($qtyX - 1), ($this->y - 1), ($qtyX + 1 + $qty_draw_width), ($this->y + $this->_general['font_size_body']) - 2);
                                                //set font to bold and up one size with white color
                                                $this->_setFont($page, 'bold', ($this->_general['font_size_body']), $this->_general['font_family_body'], $this->_general['non_standard_characters'], 'white');
                                            }
                                        }
                                        //draw qty
                                    ...
                                    if ($nameyn == 1 && $show_bundle_parent) $page->drawText($sku_name[$sku], intval($namenudge), $this->y, 'UTF-8');
                                    $store_id = Mage::app()->getStore()->getId();
                                    if ($show_bundle_parent && ($product_barcode_yn == 1 || $product_barcode_yn == 2)) {
                                        ...
                                        $productbarcodeString = $this->convertToBarcodeString($barcodeString, $barcode_type);
                                        $productbarcodeWidth = $this->parseString($barcodeString, Zend_Pdf_Font::fontWithPath($this->action_path . $font_family_barcode), $barcode_font_size);
                                        if ($grey_next_line == 0) {
                                            $page->setFillColor($white_color);
                                            $page->setLineColor($white_color);
                                        ...
                                        if ($product_barcode_bottom_yn == 1) {
                                            $this->_setFont($page, $this->_general['font_style_body'], ($product_barcode_bottom_font_size), $this->_general['font_family_body'], $this->_general['non_standard_characters'], $this->_general['font_color_body']);
                                    ...
                                    $this->y -= ($this->_general['font_size_body'] + 6);
                                }
                            }
...
//                                             }
            ...
            foreach ($product_build as $order_id => $order_build) {
                ...
                if ($condition_new_page == 1) {
                    if ($page_count == 1) {
                        $this->_setFont($page, $this->_general['font_style_subtitles'], ($this->_general['font_size_subtitles'] - 2), $this->_general['font_family_subtitles'], $this->_general['non_standard_characters'], $this->_general['font_color_subtitles']);
                        $page->drawText('-- ' . $helper->__('Page') . ' ' . $page_count . ' --', 250, ($this->_general['font_size_subtitles'] - 7), 'UTF-8');
                    }
                    $page = $this->newPage();
                    $page_count++;
                    $this->_setFont($page, $this->_general['font_style_subtitles'], ($this->_general['font_size_subtitles'] - 2), $this->_general['font_family_subtitles'], $this->_general['non_standard_characters'], $this->_general['font_color_subtitles']);
                ...
                if ($order_address_yn == 1 || $order_billing_address_yn == 1) {
                    if (isset($sku_giftwrap[$order_id]['message']) && (strlen($sku_giftwrap[$order_id]['message']) > 0)) {
                        $this->y -= 28;
                        if ($order_address_yn == 1) {
                            $this->_setFont($page, 'regular', ($this->_general['font_size_subtitles'] * 0.6), $this->_general['font_family_subtitles'], $this->_general['non_standard_characters'], $this->_general['font_color_subtitles']);
                            $page->drawText($sku_shipping_address[$order_id], 31, $this->y, 'UTF-8');
                            $this->y -= ($this->_general['font_size_subtitles'] * 0.6);
                        }
                        if ($order_billing_address_yn == 1) {
                            $this->_setFont($page, 'regular', ($this->_general['font_size_subtitles'] * 0.6), $this->_general['font_family_subtitles'], $this->_general['non_standard_characters'], $this->_general['font_color_subtitles']);
                ...
                if ($order_billing_address_yn == 0 && $order_address_yn == 0)
                    $this->y -= ($this->_general['font_size_body'] + 20);
                // order gift message.
                if ($giftmessage_yn == 1) {
                    if (isset($gift_msg_array[$order_id])) {
                        ...
                        if ($message_title_tofrom_yn == 1) {
                            ...
                            //$this->y = $this->showToFrom($message_title_tofrom_yn, $to_from, $msgX + $this->_general['font_size_body']/3, $this->y, $to_from_from, $this->_general['font_size_body'], $page);
                        ...
                        $width_title = $this->getWidthString("Gift Message: ", $this->_general['font_size_body']);
                        ...
                        $this->_setFont($page, $this->_general['font_style_body'], $this->_general['font_size_body'], $this->_general['font_family_body'], $this->_general['non_standard_characters'], $this->_general['font_color_body']);
                        $this->y = $this->drawOrderGiftMessage($gift_msg_array, $msgX + $this->_general['font_size_body'] / 3 + $width_title, $this->_general['font_size_body'], $this->y, $page);
...
//                     $dsku = $value['sku'];
                    ...
                    if ($address_label_yn == 0) {
                        if ($grey_next_line == 1) {
                            ...
                            $grey_box_y1 = ($this->y - ($this->_general['font_size_body'] / 5));
                            $grey_box_y2 = ($this->y + ($this->_general['font_size_body'] * 0.85));
                    ...
                    if ($this->y < $this->_general['font_size_subtitles'] + 5) {
                        if ($page_count == 1) {
                            $this->_setFont($page, $this->_general['font_style_subtitles'], ($this->_general['font_size_subtitles'] - 2), $this->_general['font_family_subtitles'], $this->_general['non_standard_characters'], $this->_general['font_color_subtitles']);
                    ...
                    if ($nameyn == 1 && isset($sku_name[$sku]) && $show_bundle_parent) {
                        ...
                        $font_temp = Zend_Pdf_Font::fontWithName(Zend_Pdf_Font::FONT_HELVETICA);
                        $line_width_name = $this->parseString($print_name, $font_temp, $this->_general['font_size_body']);
                    ...
                    if (($product_images_yn == 1) && isset($sku_image_paths[$sku]['path'][0]) && $show_bundle_parent) {
                        ...
                        if (($this->y - $sku_image_paths[$sku]['height'] - 3) < (20 + ($this->_general['font_size_subtitles'] * 2))) {
                            ...
                            if (isset($this->_general['second_page_start']) && $this->_general['second_page_start'] == 'asfirst') $this->y = $items_header_top_firstpage;
                            ...
                            $this->_setFont($page, 'bold', $this->_general['font_size_body'] + 2, $this->_general['font_family_body'], $this->_general['non_standard_characters'], $this->_general['font_color_subtitles']);


                            $this->y -= ($this->_general['font_size_subtitles'] * 1.5);
                            $this->_setFont($page, $this->_general['font_style_body'], $this->_general['font_size_body'], $this->_general['font_family_body'], $this->_general['non_standard_characters'], $this->_general['font_color_body']);
                        }
                        ...
                        try {
                            ...
                            if ($giftmessage_yn == 1 && $value['has_message'] == 1)
                                $this->y -= (2 * $this->_general['font_size_body'] + 20 + $sku_image_paths[$sku]['height']);
                            else
                                $this->y -= (2 * $this->_general['font_size_body'] + 35 + $sku_image_paths[$sku]['height']);
                            if (!isset($product_build_value['bundle_options_sku'])) {
                                $this->y += ($product_images_line_nudge - ($this->_general['font_size_body'] / 2));
                            }
                    ...
                    if ($giftmessage_yn == 1 && $show_bundle_parent) {
                        ...
                        if ($value['has_message'] == 1) {
                            ...
                            foreach ($value['message-content'] as $gift_msg_line) {
                                $temp_height += 2 * $this->_general['font_size_body'] + 3;
                            }
                            foreach ($value['message-content'] as $gift_msg_line) {
                                if (($this->y) < 40) {
                                    if ($page_count == 1 && isset($return_address_yn) && $return_address_yn == 0 && isset($bottom_shipping_address_yn) && $bottom_shipping_address_yn == 0) {
                    ...
                    if (isset($sku_bundle[$order_id][$sku]) && is_array($sku_bundle[$order_id][$sku])) {
                        ...
                        if ($show_bundle_parent)
                            $this->y -= ($this->_general['font_size_body']);
                        $this->_setFont($page, $this->_general['font_style_body'], ($this->_general['font_size_body'] - 2), $this->_general['font_family_body'],
                        ...
                        if ($is_split && $show_bundle_parent_yn == 1) {
                            ...
                            foreach ($sku_bundle[$order_id][$sku] as $key => $value) {
                                ...
                                $this->y -= ($this->_general['font_size_body'] - 0);

                                if ($grey_next_line_bundle == 1) {
                                    $page->setFillColor($alternate_row_color);
                                    $page->setLineColor($alternate_row_color);

                                    $grey_box_y1 = ($this->y - (($this->_general['font_size_body'] - 2) / 5));
                                    $grey_box_y2 = ($this->y + (($this->_general['font_size_body'] - 2) * 0.85));
                                ...
                                if ($qty <= 1 || $product_qty_upsize_yn == '0') {
                                    //set normal font for printing qty <=1 or when upsize qty wasn't set
                                    $this->_setFont($page, $this->_general['font_style_body'], $this->_general['font_size_body'] - 2, $this->_general['font_family_body'], $this->_general['non_standard_characters'], $this->_general['font_color_body']);
                                }else if ($qty > 1 && $product_qty_upsize_yn != '0') {
                                    $this->_setFont($page, 'bold', $this->_general['font_size_body'], $this->_general['font_family_body']-2, $this->_general['non_standard_characters'], $this->_general['font_color_body']);
                                    $qty_draw_width = $this->widthForStringUsingFontSize($qty, $page->getFont(), $this->_general['font_size_body'] - 2);
                                    if ($product_qty_upsize_yn == '1') { //boxed option
                                        $page->setFillColor($black_color);
                                        //draw box
                                        $page->drawRectangle(($qtyX - 1), ($this->y - 1), ($qtyX + 1 + $qty_draw_width), ($this->y + $this->_general['font_size_body']) - 3.5);
                                        //set font to bold and up one size with white color
                                        $this->_setFont($page, 'bold', ($this->_general['font_size_body'] - 2), $this->_general['font_family_body'], $this->_general['non_standard_characters'], 'white');
                                    }
                                }
                                //draw qty
                                $page->drawText($qty, $qtyX, ($this->y), 'UTF-8');
                                //return font to normal
                                $this->_setFont($page, $this->_general['font_style_body'], $this->_general['font_size_body'], $this->_general['font_family_body'], $this->_general['non_standard_characters'], $this->_general['font_color_body']);
                                /***************************END PRINTING BUNDLE CHILD PRODUCT QTY**********************/
                                //print child barcode
                    ...
                    if ($doubleline_yn == 2)
                        $this->y -= 2 * $this->_general['font_size_body'];
                    else if ($doubleline_yn == 1.5)
                        $this->y -= 1.5 * $this->_general['font_size_body'];
                    else if ($doubleline_yn == 3)
                        $this->y -= 3 * $this->_general['font_size_body'];
                    else
                        $this->y -= ($this->_general['font_size_body'] + 1);
                }
                // end roll_SKU
                // end each section
            ...
            if (($showcount_yn == 1) || ($showcost_yn == 1) || ($order_count_yn == 1)) {
                ...
                $shipYbox = 0;
                if ($showcount_yn == 1) {
                    $shipYbox += ($this->_general['font_size_body'] * 2);
                }

                if ($order_count_yn == 1) {
                    $shipYbox += $this->_general['font_size_body'] * 1.4;
                }
                ...
                if (($this->y - $shipYbox - $this->_general['font_size_body'] * 1.4) < 60) {
                    if ($page_count == 1) {
                        $this->_setFont($page, $this->_general['font_style_subtitles'], ($this->_general['font_size_subtitles'] - 2), $this->_general['font_family_subtitles'], $this->_general['non_standard_characters'], $this->_general['font_color_subtitles']);
                        $page->drawText('-- ' . $helper->__('Page') . ' ' . $page_count . ' --', 250, ($this->y - 15), 'UTF-8');
                    }
                ...
                $page->drawRectangle(340, ($this->y - $shipYbox - $this->_general['font_size_body'] * 1.4), ($padded_right - 2), $this->y);
                $this->_setFont($page, $this->_general['font_style_body'], $this->_general['font_size_body'], $this->_general['font_family_body'], $this->_general['non_standard_characters'], $this->_general['font_color_body']);
            }
            if ($showcount_yn == 1) {
                $this->y -= ($this->_general['font_size_body'] * 2);
                ...
                $page->setLineWidth(1);
                $this->_setFont($page, 'bold', $this->_general['font_size_body'], $this->_general['font_family_body'], $this->_general['non_standard_characters'], $this->_general['font_color_body']);
                $page->drawText($helper->__('Items') . ' : ' . $total_quantity, 370, $this->y, 'UTF-8');
            }
            if ($show_bundle_parent === true && ($total_bundle_quantity > 0)) {
                $this->y -= $this->_general['font_size_body'] * 1.4;
                $page->setFillColor($white_color);
                $page->setLineColor($black_color);
                $page->setLineWidth(0.5);
                ...
                $page->setFillColor($black_color);
                $this->_setFont($page, 'bold', $this->_general['font_size_body'], $this->_general['font_family_body'], $this->_general['non_standard_characters'], $this->_general['font_color_body']);
                $page->drawText(($helper->__('Bundles') . ' : ' . $total_bundle_quantity), 370, $this->y, 'UTF-8');
            }
            if ($order_count_yn == 1) {
                $this->y -= $this->_general['font_size_body'] * 1.4;
                $this->_setFont($page, 'bold', $this->_general['font_size_body'], $this->_general['font_family_body'], $this->_general['non_standard_characters'], $this->_general['font_color_body']);
                $page->drawText($helper->__('Orders') . ' : ' . $order_count, 370, $this->y, 'UTF-8');
            }
            if (($showcost_yn == 1) && ($total_cost > 0)) {
                $this->y -= $this->_general['font_size_body'] * 1.4;
                $page->drawText($helper->__('Total cost') . ' : ' . $currency_symbol . "  " . ($total_cost), 360, $this->y, 'UTF-8');
            }
            if ($printdates == 1) {
                $this->y -= $this->_general['font_size_body'] * 1.4;
