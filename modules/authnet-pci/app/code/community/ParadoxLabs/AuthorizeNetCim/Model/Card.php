...
class ParadoxLabs_AuthorizeNetCim_Model_Card extends ParadoxLabs_TokenBase_Model_Card
{
	...
	{
		$acceptJsKey	= $this->getInfoInstance()->getAdditionalInformation('acceptjs_key');
		...
		if( !empty( $acceptJsKey ) && !empty( $acceptJsValue ) ) {
			...
			$this->getInfoInstance()->setAdditionalInformation( 'acceptjs_key', null );
			$this->getInfoInstance()->setAdditionalInformation( 'acceptjs_value', null );
			
			if( $this->getInfoInstance()->getId() > 0 ) {
	...
	protected function _setPaymentInfoOnUpdate( $gateway )
	{
		 ...
		 * Send Accept.js nonce instead of payment info, if we have it.
		 */
		if( !empty( $acceptJsKey ) && !empty( $acceptJsValue ) && $this->getInfoInstance()->getBaseAmountAuthorized() <= 0 ) {
			$gateway->setParameter( 'dataDescriptor', $acceptJsKey );
			$gateway->setParameter( 'dataValue', $acceptJsValue );
		...
		elseif( strlen( $this->getInfoInstance()->getCcNumber() ) >= 12 ) {
			$gateway->setParameter( 'cardNumber', $this->getInfoInstance()->getCcNumber() );
		}
		else {
