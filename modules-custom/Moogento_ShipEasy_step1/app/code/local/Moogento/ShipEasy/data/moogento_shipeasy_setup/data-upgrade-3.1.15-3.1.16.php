...
foreach ($configData as $path => $value) {
    if (stripos(strrev($path), 'wohs_') === 0) {
        $config->saveConfig('moogento_shipeasy/grid/' . $path, (int)!$value);
    }
}
...
$config->saveConfig('moogento_shipeasy/grid/product_image_max_number', Mage::getStoreConfig('moogento_shipeasy/grid/image_max_number'));
