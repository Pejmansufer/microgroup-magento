...
class Moogento_Pickpack_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function getParentProId($product_id){
		if ($image_product = $this->getProduct($product_id)) {
			...
			if (($has_real_image_set !== true) && (is_object($image_product)) && ($image_product->getTypeId() == "simple")) // if is child (not parent)
			{
	...
	public function editFlag($id, $orderid, $invoice_printed=0, $pack_printed=0, $separate_printed=0, $combined_printed=0, $manual_invoice_printed=0, $manual_pack_printed=0,$manual_separate_printed=0, $manual_combined_printed=0){
		$flagautoaction = Mage::getModel('pickpack/flagautoaction');
		try{
			$flagautoaction->load($id);
			$flagautoaction->setOrderid($orderid)
						->setInvoice_printed($invoice_printed)
						->setPack_printed($pack_printed)
						->setSeparate_printed($separate_printed)
						->setCombined_printed($combined_printed)
						->setManual_invoice_printed($manual_invoice_printed)
						->setManual_pack_printed($manual_pack_printed)
						->setManual_separate_printed($manual_separate_printed)
						->setManual_combined_printed($manual_combined_printed);
			$flagautoaction->save();
			return;
		}
		catch(Exception $e){
	...
	public function newFlag($orderid, $invoice_printed=0, $pack_printed=0, $separate_printed=0, $combined_printed=0, $manual_invoice_printed=0, $manual_pack_printed=0,$manual_separate_printed=0, $manual_combined_printed=0){
		$flagautoaction = Mage::getModel('pickpack/flagautoaction');
		try{
			$flagautoaction->setOrderid($orderid)
						->setInvoice_printed($invoice_printed)
    ...
    public function processRelatedOrders($orderIds)
    {
        ...
        foreach($orderIds as $single_order_id)
        {
            ...
            foreach($order_collection as $o_model)
            {
                ...
                $order_printgroup = Mage::getModel('pickpack/printgroup')->getCollection()
                              ->addFieldToFilter('order_increment_id',$order_increment_id)
                                ;
