...
class Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices_Labelzebra extends Moogento_Pickpack_Model_Sales_Order_Pdf_Invoices
{
    ...
    private function getAttributeValueZebra($shelving_real_attribute, $item, $shelving_real_attribute_yn){
        ...
        if ($attributeName == '%category%') {
            ...
            $attributeValue = $this->getProductAttributeValue($product,$attributeName);
        }
        return $attributeValue;
    }
    private function getCapitalize($value)
    {
        $valueC = '';
        switch ($this->_config['capitalize_zebra_yn']) {
            case 0:
                ...
                break;
            case 1:
                $valueC = ucwords($value);
//                please recheck this function
//                $valueC = $this->capitalAddress($valueC);
                break;
            case 2:
                $valueC = strtoupper($value);
                break;
        }
        return $valueC;
    }
    ...
    public function getLabelzebra($orders = array())
    {
        ...
        10x14         720x1008
        */
        // $paper_width     = $this->_getConfig('paper_width', 595, false,'label');
        // $paper_height     = $this->_getConfig('paper_height', 842, false,'label');
        ...
        if ($rotate_label == 0) {
            ...
            $page                  = $pdf->newPage($page_demension);
            $rotate_point[0]       = 0;
            $rotate_point[1]       = 0;
        ...
        $next_y            = $current_y;
        $flag_label_height = 0;
        $rotate_label_temp = $rotate_label;
...
//                    $this->_config['nudge_demension_zebra'][1]  : the height of page
                ...
                foreach ($shippingAddressArray as $i => $value) {
                    ...
                    if($value != ''){
                        $value      = $this->getCapitalize($value);
                ...
                $barcode_x       = isset($address_line_x) ? $address_line_x : 0;
                ...
                if ($show_address_barcode_yn == 1 && $zipcode != '') {
                    ...
                    $barcodeString     = $this->convertToBarcodeString($zipcode, $barcode_type);
                ...
                if($show_product_list == 1 && ($separate_zebra_page_yn == 0 || $separate_zebra_page_yn == 1)){
                    ...
                    foreach ($itemsCollection as $item) {
                        ...
                        $product_simple = $this->_getProductFromItem($item);
                        ...
                        if($combine_product_line == 1){
                            ...
                            elseif($store_view == "specificstore" && $specific_store_id != "") {
                                ...
                                if ($sku_name[$sku] == '') $name_item = trim($item->getName());
                        ...
                        if (($show_product_sku == 1 || $show_product_sku == 'configurable') && $combine_product_line == 0) {
                            ...
                            if($show_product_qty == 1) $this->drawText($page,' x ', ($qtyX + $skuX)/2 - 2, $temp_y, 'UTF-8');
                            ...
                            if($trim_product_sku_yn == 1){
                                ...
                                $lineName = 0;
                                $multiline_name = wordwrap($sku_item, $max_chars_name, "\n");
                                $token = strtok($multiline_name, "\n");
                                if ($token != false) {
                                    while ($token != false) {
                                        $this->drawText($page,$sku_item, $skuX, $temp_temp_y, 'UTF-8');
                            ...
                            if($temp_temp_y < $min_temp_y) $min_temp_y = $temp_temp_y;
                            ...
                            if (count($children)) {
                                ...
                                foreach ($children as $child) {
                                    ...
                                    if (($shelving_real_attribute_yn == 1 || $shelving_real_attribute_yn == 'configurable') && $shelving_real_attribute != '') {
                                        $attributeName = $shelving_real_attribute;
                                        //$product        = Mage::getModel('catalog/product')->load(trim($sku_b), 'sku');
                                        ...
                                        //     $category_label = implode(', ', $categsToLinks);
                                        //     $attributeValue = $category_label;
                                        // } elseif ($product->getData($attributeName)) {
                        ...
                        $temp_y = $temp_y - $font_size_product;
                        if($final_zebra_summary == 1){
                            $summaryattributeValue = $this->getAttributeValueZebra($summary_custom_attribute, $item, $shelving_real_attribute_yn);//$this->getProductAttributeValue($product,$summary_custom_attribute);
                ...
                if($final_zebra_summary == 1){
                    ...
                    // else{
                    // foreach($item_zebra as $item_each){
                    // if(isset($summary_zebra[$item_each["group"]])){
                    // $summary_zebra[$item_each["group"]]["total_weight"] += $item_each["total_weight"];
                    // $summary_zebra[$item_each["group"]]["total_items"] += $item_each["total_items"];
                    // $summary_zebra[$item_each["group"]]["total_orders"] += $item_each["total_orders"];
                    // }
                    // else{
                    // $summary_zebra[$item_each["group"]] = $item_each;
                    // }
                    // }
                    // }
                }
            ...
            if ($this->_config['add_cn22_page_yn']){
                ...
                if ($print_cn22_page_yn){
                    ...
                    $this->_setFontRegular($page, ($this->_config['font_size_return_label_side'] - 0));
                    $this->_setFont($page, $this->_config['font_style_return_label_side'], ($this->_config['font_size_return_label_side'] - 0), $this->_config['font_family_return_label_side'], $this->_non_standard_characters, $this->_config['font_color_return_label_side']);
    ...
    private function printReturnAddressOnSameLabel($page,$address_line_x, $address_line_y){
        ...
        $this->_setFont($page, $this->_config['font_style_return_label_side'], $this->_config['font_size_return_label_side'], $this->_config['font_family_return_label_side'], $this->_non_standard_characters, $this->_config['font_color_return_label_side']);
        ...
        if (preg_match('~^' . $from_text . '~', $return_address_lines[0])) {
            ...
            $this->_setFont($page, $this->_config['font_style_return_label_side'], ($this->_config['font_size_return_label_side'] - $return_address_title_fontsize), $this->_config['font_family_return_label_side'], $this->_non_standard_characters, $this->_config['font_color_return_label_side']);
