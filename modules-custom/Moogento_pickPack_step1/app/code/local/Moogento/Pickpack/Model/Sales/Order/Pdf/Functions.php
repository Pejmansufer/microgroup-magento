...
function widthForStringUsingFontSize($string, $font, $font_size)
{
	$string = trim($string);
	if($string == '') return '';
	...
	if(!is_object($font))
	{
		$font_path = Mage::getBaseDir('app') . '/code/local/Moogento/Pickpack/Model/Sales/Order/Pdf/'.$font.'.ttf';
...
function stringBreak($string, $max_length, $font_size, $font='helvetica')
{
	$string = trim($string);
	if($string == '') return '';
	$test_string = '';
	...
	$string_width = widthForStringUsingFontSize($test_string, $font, $font_size);
