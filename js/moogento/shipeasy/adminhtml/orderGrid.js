...
jQuery(document).ready(function ($) {
    ...
    $('#anchor-content').on('click', 'td.szy_custom_col', function (e) {
        ...
        if (e.target != this){ 
            if (!$(e.target).hasClass('restriction')) return;
        }

        if (!bill_ship_form_changing) {
            $(this).find('div.chosen-container.chosen-container-single').show();
            $(this).find('.custom_color').hide();
            $(this).find('select.chosen-select').trigger('chosen:open');
        }
    });
    ...
    $('#anchor-content').on('change', '.szy_custom_col select.chosen-select', function (e) {
        ...
        if($selectElem.val() == "New Value") {
            $("#loading-mask").addClass('hidden_el');
            var $parent_of_elem = $selectElem.parent();
            var $children_parent = $parent_of_elem.children();
            $children_parent.each(function (elem) {
            ...
            $parent_of_elem.find(".input_custom_value").show().removeClass('hidden_el').focus();            
        } else {
            ...
            .done(function (data) {
                $selectElem.siblings('.custom_color,.custom_flag').first().replaceWith(JSON.parse(data).html);
                hideSpinner($parent_of_elem);
                $("#loading-mask").removeClass('hidden_el');
                $children_parent.each(function (elem) {
                    $(this).removeClass('hidden_el');
                });
            });
        }
    });
